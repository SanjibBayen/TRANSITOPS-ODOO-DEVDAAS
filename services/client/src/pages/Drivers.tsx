...
import { 
  Users, CheckCircle2, ShieldCheck, Search, SlidersHorizontal, Plus, 
  Trash2, Edit3, X, Mail, Phone, Award, Calendar, AlertTriangle, RefreshCw
} from 'lucide-react';
import { toast } from 'sonner';

export const Drivers: React.FC = () => {
  const { 
    drivers, isLoading, metrics, 
    loadDrivers, createDriver, updateDriver, updateDriverStatus 
  } = useDrivers();
  ...
  const handleOpenAdd = () => {
    ...
    setForm(prev => ({
      ...prev,
      license_number: `MH-${new Date().getFullYear()}-${String(Math.floor(1000000 + Math.random() * 9000000))}`,
    }));
    setIsModalOpen(true);
  };
...
const licenseWarning = (expiry: string) => {
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs font-semibold text-gray-500 dark:text-zinc-400">Manage driver profiles, licenses, and compliance.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total Drivers', value: metrics.total, icon: Users, color: 'text-[#714B67]', bg: 'bg-purple-50' },
          { label: 'Available', value: metrics.available, icon: CheckCircle2, color: 'text-green-600', bg: 'bg-green-50' },
          { label: 'On Trip', value: metrics.onTrip, icon: ShieldCheck, color: 'text-blue-600', bg: 'bg-blue-50' },
        ].map((card, i) => (
      ...
      {/* Filters */}
      <div className="rounded-xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 p-4 flex flex-col md:flex-row items-center gap-3">
        <div className="relative w-full">
          <Search className="absolute left-3 top-2.5 h-4 w-4 text-gray-500" />
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by name, license, or phone..." className="w-full rounded-lg border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-800 py-2 pl-9 pr-4 text-xs focus:border-[#714B67] focus:outline-none focus:ring-1 focus:ring-[#714B67] transition-all font-medium" />
        </div>
      ...
      <div className="rounded-xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[#eae8e7] dark:divide-zinc-800">
              {isLoading ? (
                <tr><td colSpan={6} className="py-12 text-center"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#714B67] mx-auto" /></td></tr>
              ) : filteredDrivers.length === 0 ? (
                <tr><td colSpan={6} className="py-12 text-center text-xs font-bold text-gray-500">No drivers found.</td></tr>
              ) : (
                filteredDrivers.map((d) => (
                  <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800 text-xs font-medium transition-all group">
                    <td className="py-3.5 px-4">
                      <div className="font-extrabold text-[#1b1c1c] dark:text-zinc-100">{d.name}</div>
                      <div className="text-[10px] text-gray-500">{d.total_trips || 0} trips completed</div>
                    </td>
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-1.5">
                        <span className="font-mono font-bold">{d.license_number}</span>
                        {licenseWarning(d.license_expiry)}
                      </div>
                      <div className="text-[10px] text-gray-500">{d.license_category} | Exp: {d.license_expiry ? new Date(d.license_expiry).toLocaleDateString() : 'N/A'}</div>
                    </td>
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-1 text-[10px]"><Phone className="h-3 w-3" />{d.phone}</div>
                      {d.email && <div className="flex items-center gap-1 text-[10px] text-gray-500"><Mail className="h-3 w-3" />{d.email}</div>}
                    </td>
                    <td className="py-3.5 px-4">
                      <span className={`font-bold ${(d.safety_score || 100) >= 80 ? 'text-green-600' : 'text-red-600'}`}>
                        ★ {d.safety_score || 100}/100
                      </span>
                    </td>
                    <td className="py-3.5 px-4">{statusBadge(d.status)}</td>
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleOpenEdit(d)} className="p-1.5 rounded bg-purple-50 text-[#714B67] hover:bg-[#714B67] hover:text-white transition-all"><Edit3 className="h-3.5 w-3.5" /></button>
                        <button onClick={() => handleDelete(d.id)} className="p-1.5 rounded bg-red-50 text-red-600 hover:bg-red-600 hover:text-white transition-all"><Trash2 className="h-3.5 w-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))
        ...
        <div className="p-4 bg-gray-50 dark:bg-zinc-950 border-t border-gray-200 dark:border-zinc-800 text-xs text-gray-500 font-semibold">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 shadow-2xl max-w-lg w-full">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 max-h-[65vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase text-gray-500">License Category</label>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" disabled={isSubmitting} className="px-4 py-2 rounded-xl bg-[#714B67] text-white text-xs font-bold disabled:opacity-50">
                  {isSubmitting ? 'Saving...' : editingDriver ? 'Update' : 'Create'}
                </button>
              </div>
            </form>
          </div>
